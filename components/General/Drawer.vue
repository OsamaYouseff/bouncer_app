...
<template lang="pug">
  TransitionRoot(as="template" :show="showDrawer")
    Dialog(class="relative z-50 bg-black h-screen overflow-y-auto" @close="emit('update:closeDrawer')")
      ...
      div( class="fixed inset-0overflow-x-hidden")
        div( class="absolute inset-0 overflow-x-hidden")
          div(class="pointer-events-none fixed inset-y-0 left-0 flex max-w-full")
            TransitionChild( enter-to="translate-x-0" leave-from="translate-x-0" as="template"
            enter="transform transition ease-in-out duration-300 sm:duration-500"
            enter-from="-translate-x-full" leave-to="-translate-x-full"
            leave="transform transition ease-in-out duration-300 sm:duration-500")
            
              DialogPanel( class="pointer-events-auto max-w-[310px]")
                div( class="flex h-screen overflow-y-auto flex-col overflow-x-hidden bg-white py-8 shadow-xl")
                  div( class="relative mt-6 flex-1 px-4 sm:px-6 min-h-fit") <slot />
